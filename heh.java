...
public class Robot extends IterativeRobot {
	...
	*/
	...
	double imuYawLast;
	double motorPower;
	double motorPowerLast;
	double Kp = 2.5;
	double Kd = 2;
	double Ki = .03;
	double currentIntegral;
	boolean imuBig = false;
	double error;
	double errorLast; 
...
public void imuRightPid() {
	...
	if (imuBig == true) {
		if (imuYaw >= 0 && imuYaw <= 90) {
			imuYaw += 360;
		}
	}
	
	error = imuYawIdeal - imuYaw;
	...
	if (i != 0) {
		...
		/*if (motorPower < 50) {
			currentIntegral += motorPower * .02;
			motorPower += currentIntegral * Ki;
		}
		motorPower += error / .02 * Kd;
		*/
		

		motorPower = error * Kp;
		
		motorPower -= ((errorLast - error) / .02 ) * Kd;
	...
	System.out.println("Current Integral: " + currentIntegral);
	System.out.println("Derivative: " + (errorLast - error / .02));
	System.out.println("Motor Power: " + motorPower);
	
	
	/*
	if( (imuYaw >= imuYawIdeal -2.5) && (imuYaw <= imuYawIdeal + 2.5)) {
		leftMotor1.set(ControlMode.PercentOutput, 0);
		leftMotor2.set(ControlMode.PercentOutput, 0);
		rightMotor1.set(ControlMode.PercentOutput, 0);
		rightMotor2.set(ControlMode.PercentOutput, 0);
	...
	*/
		leftMotor1.set(ControlMode.PercentOutput, -motorPower/100);
		leftMotor2.set(ControlMode.PercentOutput, -motorPower/100);
		rightMotor1.set(ControlMode.PercentOutput, -motorPower/100);
		rightMotor2.set(ControlMode.PercentOutput, -motorPower/100);
	//}
} 

public void stupidLeft() {
	
	imuYaw = (imuTable.getEntry("yaw").getDouble(0));
	int imuEndDegree = 0;
	if(i == 0) {
		imuYawInitial = imuYaw;
    imuEndDegree = (int) imuYaw - 90;
    if(imuEndDegree < 0){
      imuEndDegree = imuEndDegree + 360;
    }
  ...
  //how much into the turn
  int imuYawDifference = (int) (Math.abs((imuEndDegree - imuYaw)) % 90);

  int correctionFactor = (int) (.23 / ((imuYawDifference / 10) + 1));

  leftMotor1.set(ControlMode.PercentOutput, .25 - correctionFactor);
  leftMotor2.set(ControlMode.PercentOutput, .25 - correctionFactor);
  rightMotor1.set(ControlMode.PercentOutput, .25 - correctionFactor);
  rightMotor2.set(ControlMode.PercentOutput, .25 - correctionFactor);

  if (imuYawDifference > 89){
    leftMotor1.set(ControlMode.PercentOutput, 0);
    leftMotor2.set(ControlMode.PercentOutput, 0);
    rightMotor1.set(ControlMode.PercentOutput, 0);
    rightMotor2.set(ControlMode.PercentOutput, 0);
...
public void imuLeftTurn() {
	...
	System.out.println("Difference: ");
			...
			//start of my code
	if (imuYaw <= imuYawIdeal + 45 && imuYaw >= imuYawIdeal - 45) {
		System.out.println("Within 45 degrees");
		if(imuYaw <= imuYawIdeal + 30 && imuYaw >= imuYawIdeal - 30) {
			System.out.println("Within 30 degrees");
			if (imuYaw <= imuYawIdeal + 15 && imuYaw >= imuYawIdeal - 15) {
				...
				if (imuYaw <= imuYawIdeal + 5 && imuYaw >= imuYawIdeal - 5) {
					...
					if (imuYaw <= imuYawIdeal + 1 && imuYaw >= imuYawIdeal - 1) {
						...
						leftMotor1.set(ControlMode.PercentOutput, 0.15);
						leftMotor2.set(ControlMode.PercentOutput, 0.15);
						rightMotor1.set(ControlMode.PercentOutput, 0.15);
						rightMotor2.set(ControlMode.PercentOutput, 0.15);
					}
				} else {
					leftMotor1.set(ControlMode.PercentOutput, 0.15);
					leftMotor2.set(ControlMode.PercentOutput, 0.15);
		...
		leftMotor2.set(ControlMode.PercentOutput, 0.25);
		rightMotor1.set(ControlMode.PercentOutput, 0.25);
		rightMotor2.set(ControlMode.PercentOutput, 0.25);
	} 
}
